...
const AdminPanel = () => {
  ...
  // Fonctions (Logout, GenerateLink, ToggleSidebar)
  ...
  // Fonction de mise à jour du statut (Appel API)
  const updateReview = async (id, newStatus) => {
      ...
      else { console.warn("Token manquant pour màj avis."); }
        ...
        return (
          <div className="admin-dashboard">
            ...
            <div className="dashboard-stats">
              <div className="stat-card"><h3>{isLoading ? '...' : pendingReviews.length}</h3><p>{t('admin.pending_reviews')}</p></div>
  ...
  // --- Rendu JSX principal AVEC LA SIDEBAR ---
  return (
    // Le div principal avec la classe pour gérer l'état replié
    <div className={`admin-panel ${sidebarCollapsed ? 'sidebar-collapsed' : ''}`}>
      ...
      <div className="admin-sidebar">
        ...
        <div className="admin-nav">
          <button className={`nav-item ${activeTab === 'dashboard' ? 'active' : ''}`} onClick={() => setActiveTab('dashboard')}>
             <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><rect x="3" y="3" width="7" height="9" rx="2" stroke="currentColor" strokeWidth="2"/><rect x="14" y="3" width="7" height="5" rx="2" stroke="currentColor" strokeWidth="2"/><rect x="14" y="12" width="7" height="9" rx="2" stroke="currentColor" strokeWidth="2"/><rect x="3" y="16" width="7" height="5" rx="2" stroke="currentColor" strokeWidth="2"/></svg>
          ...
          <button className={`nav-item ${activeTab === 'reviews' ? 'active' : ''}`} onClick={() => setActiveTab('reviews')}>
             <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M11.0489 3.92705C11.3483 3.00574 12.6517 3.00574 12.9511 3.92705L14.0206 7.21885C14.1545 7.63087 14.5385 7.90983 14.9717 7.90983H18.4329C19.4016 7.90983 19.8044 9.14945 19.0207 9.71885L16.2205 11.7533C15.8684 12.0079 15.7234 12.4593 15.8572 12.8713L16.9268 16.1631C17.2261 17.0844 16.1717 17.8506 15.388 17.2812L12.5878 15.2467C12.2356 14.9921 11.7644 14.9921 11.4122 15.2467L8.61204 17.2812C7.82833 17.8506 6.77385 17.0844 7.0732 16.1631L8.14277 12.8713C8.27665 12.4593 8.13162 12.0079 7.77946 11.7533L4.97926 9.71885C4.19555 9.14945 4.59834 7.90983 5.56712 7.90983H9.02832C9.46154 7.90983 9.8455 7.63087 9.97937 7.21885L11.0489 3.92705Z" stroke="currentColor" strokeWidth="2"/></svg>
             <span className="nav-text">
               {t('admin.reviews')}
               {!isLoading && pendingReviews.length > 0 && ( <> {' '} ({pendingReviews.length}) </> )}
             </span>
          </button>
          <button className={`nav-item ${activeTab === 'content' ? 'active' : ''}`} onClick={() => setActiveTab('content')}>
             <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M14 7H4M14 12H4M14 17H4M19 7V17M17 9H21M17 15H21" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
             <span className="nav-text">{t('admin.content')}</span>
          </button>
          <button className={`nav-item ${activeTab === 'media' ? 'active' : ''}`} onClick={() => setActiveTab('media')}>
             <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M2 6C2 4.89543 2.89543 4 4 4H20C21.1046 4 22 4.89543 22 6V18C22 19.1046 21.1046 20 20 20H4C2.89543 20 2 19.1046 2 18V6Z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/><path d="M8.5 11C9.32843 11 10 10.3284 10 9.5C10 8.67157 9.32843 8 8.5 8C7.67157 8 7 8.67157 7 9.5C7 10.3284 7.67157 11 8.5 11Z" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/><path d="M22 14L17.5 11L9 17" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
          ...
          <button className={`nav-item ${activeTab === 'marketing-integrations' ? 'active' : ''}`} onClick={() => setActiveTab('marketing-integrations')}>
             <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3 3V21H21" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/><path d="M7 14L11 10L15 14L21 8" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
          ...
          <button className={`nav-item ${activeTab === 'wordpress' ? 'active' : ''}`} onClick={() => setActiveTab('wordpress')}>
             <svg width="24" height="24" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M12 2C6.48 2 2 6.48 2 12C2 17.52 6.48 22 12 22C17.52 22 22 17.52 22 12C22 6.48 17.52 2 12 2ZM3.68 12C3.68 7.41 7.41 3.68 12 3.68C16.59 3.68 20.32 7.41 20.32 12C20.32 16.59 16.59 20.32 12 20.32C7.41 20.32 3.68 16.59 3.68 12ZM12 6.8C9.1 6.8 6.8 9.1 6.8 12C6.8 14.9 9.1 17.2 12 17.2C14.9 17.2 17.2 14.9 17.2 12C17.2 9.1 14.9 6.8 12 6.8Z" fill="currentColor"/></svg>
      ...
      <div className="admin-content-wrapper">
        ...
        {/* Zone où le contenu de la section active est rendu */}
